...
- `.github/workflows/ci.yml`: macOS + Linux CI for tests/vet.
- `.github/workflows/release.yml`: Tag-based release workflow (GoReleaser).
- `.goreleaser.yml`: Release + Homebrew tap + Linux tarball configuration.
...
- GitHub Actions runs `go mod tidy`, `go test ./...`, and `go vet ./...` on macOS + Linux.
...
- Tagged releases (`v*`) run GoReleaser to publish GitHub releases (macOS + Linux tarballs) and update the Homebrew tap (requires `HOMEBREW_TAP_GITHUB_TOKEN` secret).
